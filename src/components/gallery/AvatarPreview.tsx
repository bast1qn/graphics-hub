...
export default function AvatarPreview({ asset, fullSize = false }: AvatarPreviewProps) {
  const renderAvatar = () => {
    switch (asset.id) {
      ...
      case 'orb-indigo':
        return (
          <div className={`${asset.styleClass}`}>
      ...
      case 'grad-purple':
        return (
          <div className={`${asset.styleClass}`} style={{ background: 'linear-gradient(135deg, #4f46e5, #a855f7)' }}>
            <div className="grad-text">B</div>
          </div>
        );
      case 'grad-dark':
        return (
          <div className={`${asset.styleClass}`} style={{ background: 'linear-gradient(135deg, #0f172a, #334155)' }}>
            <div className="grad-text">B</div>
          </div>
        );
      case 'sig-cursive':
  ...
  // Full size for modal (800x800)
  if (fullSize) {
    return (
      <div id={`avatar-${asset.id}`} className="avatar-content">
        {renderAvatar()}
      </div>
    );
  }

  // Card preview (just the content, wrapper handles scaling)
  return (
